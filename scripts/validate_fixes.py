#!/usr/bin/env python3
"""Validate the accuracy improvements after fixes."""
import sys, warnings
...
def run_test(ticker, market_name, strategy_name, description):
    ...
    # Show feature count (should be lower now with non-stationary removed)
    ...
    if fs_method != "none":
        ...
        X = X[selected]
        print(f"  After FS: {X.shape[1]} features: {selected}")
    ...
    if len(X) < init_train + val_cfg.get('test_days', 42):
    ...
    if num_classes == 2:
        print(f"\n  {classification_report(all_true, all_pred, target_names=['DOWN', 'UP'])}")
    else:
        print(f"\n  {classification_report(all_true, all_pred, target_names=['DOWN', 'FLAT', 'UP'])}")
...
# Run tests
print("=" * 70)
print("POST-FIX ACCURACY VALIDATION")
print("=" * 70)
...
print("SUMMARY")
print("=" * 70)
print(f"{'Ticker':<12} {'Market':<10} {'Acc':>7} {'Base':>7} {'Lift':>7} {'Feats':>6}")
print("─" * 55)
for r in results:
    print(f"{r['ticker']:<12} {r['market']:<10} "
          f"{r['acc']:>6.1%} {r['baseline']:>6.1%} "
          f"{r['lift']:>+6.1%} {r['features']:>6}")
...
if results:
    ...
    print("─" * 55)
    print(f"{'AVERAGE':<22} {avg_acc:>6.1%} {avg_base:>6.1%} {avg_lift:>+6.1%}")
